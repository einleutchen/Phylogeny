...
Therefore, to select homologous
proteins, I set the threshold for percentage of identity at 70% and query coverage
90% to obtain diverged-but-whole sequences. The cutoffs for selection of
homologous genes, that are 70% identity and 50% coverage. When targetting higher cutoffs for genes, I could not obtain any set of sequences for further analysis,
plus to avoid collecting sets of fragmented sequences (query coverage < 40) I
think these cutoffs are reasonable. 
The E-value for protein is 1e-05 while for
...
IQTree2 eventually returns one of the files with the best tree that
...
| Tree | Protein  | Nucleotide 3 |
| -------- | -------- | -------- |
1 | Glycine-tRNA ligase | Elongation factor Tu |
2 | RNA polymerase | Sporulation factor | Elongation factor Tu |
3 | 50S ribosomal protein L11 | DNA polymerase subunit B |
4 | 50S ribosomal protein L1 | (not specified) |
5 | 50S ribosomal L7/L12 | (not specified) |
Concat | "Glycine-tRNA ligase(cov: 39%, ident: 97%)" | "DNA polymerase(cov: 44%, ident: 98%)
